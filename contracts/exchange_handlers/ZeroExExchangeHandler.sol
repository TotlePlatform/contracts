...
contract ZeroExExchangeHandler is
    TotleControl,
    ExchangeHandler,
    AllowanceSetter
{
    ...
    function getAmountToGive_(OrderData memory data)
        internal
        view
        returns (uint256 amountToGive)
    {
        ...
        uint256 feeAssetAvailable = getAssetDataAvailable(
            ZRX_ASSET_DATA,
            data.makerAddress
        );
        ...
        returns (uint256 amountSpentOnOrder, uint256 amountReceivedFromOrder)
    {
        ...
        LibFillResults.FillResults memory results = exchange.fillOrder(
            ...
            Math.min(
                targetAmount,
                Math.min(availableToSpend, getAmountToGive_(data))
            ),
            data.signature
        );
